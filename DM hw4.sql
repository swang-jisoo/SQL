...
/* inner join command
SELECT C.CourseID, C.CourseName
FROM (Course_T C inner join Qualified_T Q on C.CourseID = Q.CourseID) inner join Faculty_T
F on Q.FacultyID = F.FacultyID
WHERE F.FacultyName = 'Berndt';
*/
--Q2 Ch7 Ex1(c)
--Display the class roster, including student name, for all students enrolled in section 2714 of
ISM 4212
...
SELECT C.CourseID, C.CourseName, ST.StudentID, ST.StudentName, SE.SectionNo,
SE.Semester
FROM ((Student_T ST inner join Registration_T R on ST.StudentID = R.StudentID) inner join
Section_T SE on R.SectionNo = SE.SectionNo) inner join Course_T C on SE.CourseID =
C.CourseID
...
/* inner join command
SELECT COUNT( StudentID )
FROM Section_T S inner join Registration_T R on S.SectionNo = R.SectionNo AND
...
FROM Order_T);
--Q8 List the names and number of employees supervised for all the supervisors who supervise
more than two employees. No need to re-label any column.
...
HAVING COUNT( E.EmployeeID ) > 2;
*/
--Q9 Display each item ordered for order #1, its standard price, and total price for each item
ordered
SELECT P.ProductID, ProductDescription, ProductStandardPrice, Sum( OrderedQuantity *
ProductStandardPrice ) AS TotalPrice
