-- Practice questions of previous AITP DB contests (2012-16)
-- 2017/04/01
-- Jisoo Lee 

-- PART 1: Database Design
-- Keep reminding the following points:
-- 1) logic
-- 2) Many Entity >> immediately insert Foreign Key (FK)!!!
-- 3) M:M attribute e.g. quantity
-- 4) recheck relationship & FK at the end
-- 5) save as PDF

-- PART 2: Database Implementation SQL
-- Keep reminding the following points:
-- 1) ERD vs. create table code
-- 2) does 'create table/insert record' commend works?***** 
...
/* 2016 Question 
Q. The management wants a view called PendingTripsView, which contains 
   only those trips that have a status of ‘pending’.:
 	...
 	e. ordered by Customer name, and then by Tube Requested
>> Summary:
...
/* 2015 Question 
1. The management wants a new table (not a view but a table) called 
   WoodProducts, which contains the same columns as the Products table, 
   except that only furniture made from wood are included. This includes:  
   maple, birch, walnut, cherry, and mahogany.  
*/
...
/* 2015 Question 
2. Create the SQL code needed to put the following facts into the database, 
   ...
   (Note: the number of units used of each raw material will be in the 
   format 99.9)
*/
...
CONSTRAINT prod_Prod_Mat_id_FK FOREIGN KEY (Prod_Id) 
     REFERENCES Products(Prod_ID),
CONSTRAINT Mat_Prod_Mat_id_FK FOREIGN KEY (Material_ID) 
     REFERENCES raw_material(Material_ID));

SELECT prod_id 
FROM Products 
WHERE UPPER(prod_description) LIKE UPPER('Filing Cabinet'); 
-- result: 82P2

SELECT * 
FROM raw_material
WHERE UPPER(material_description) LIKE UPPER('%steel%')
OR UPPER(material_description) LIKE UPPER('%track%'); 
-- result: tubing 6; guage 7; epoxy 8; track 9
...
SELECT  p.prod_description AS "Product", 
        pm.Unit_Used || m.unit_of_measure || ' of ' || m.material_description 
	AS "Raw Materials"
...
-- Query 1
/* 2016 Question
1. How much was the cost for transaction #5? Display Transaction ID 
...
/* 2015 Question
1. Write a query that will calculate how much money it costs to produce the 
filing cabinet.  
*/
...
/* 2014 Question (MYSQL)
1. This query will select information about volunteers and their assigned animals. 
...
SELECT concat('(', left(vol_phone,3),')',mid(vol_phone,4,3),'-',right(vol_phone,4), 
              lpad(concat(vol_first,' ', vol_last),20,'.')) 
	      AS 'Volunteer Contact Information',
...
-- Query2
/* 2016 Question
2. List the Customer ID, Name, City, State for customers who had rental 
   transactions processed for trips that started in both the months of 
   June 2014 and July 2014 (i.e. had at least one Trip start date in June 
   2014 and at least one in July 2014).
>> Summary:
	- limit: tripstartdate: month = BOTH 6 and 7; year = 2014
	- columns: custid, fname lname as customer name, 
	  city state as location, phone, email, tripstartdate
*/
...
/* 2015 Question
2. The Pine Valley Furniture Company gives a 10% discount to companies 
   who spend more than $100,000. Write a query that will provide the total 
   amount spent on all orders that are currently in the Orders table.  
...
/* 2014 Question
2. The Rescue Shelter needs to find Adopters and Volunteers who have the 
   same zip code so that homes can be found in a nearby vicinity. 
   List all of the Adopters and Volunteers who have the same zip code. 
   Also list the number of wishes for each adopter. Only include those 
   adopters who have 2 or more wishes. Order by column 1.
*/

SELECT CONCAT(adopter_first, ' ',adopter_last, ' has ', COUNT(wish_id), ' wishes') 
AS 'Adopter and Wishlist', 
CONCAT(vol_first, ' ', vol_last) AS 'Volunteer', 
adopter_zip AS 'Common Zip Code'
FROM adopter a
...
-- Query3
/* 2016 Question
3. Create a query that will list a phrase giving person capacity, 
   ‘One Person’, ‘Two Persons’, etc., the name of the tube type, the 
   rental charge with 6% tax, and the charge per person.
>> Summary:
	- limit: case person capacity
	- columns: personcapacity as can hold, tubename, rentalcharge with 6%tax, 
	  rentalcharge per person
*/
...
/* 2015 Question
3. Write the SQL code to give the number of orders along with the total 
   number of items that were in all orders, grouped by the order’s origination 
   (‘O’ for ‘Online’, ‘P’ for ‘Phone’, and ‘M’ for ‘Mail’).  
...
/* 2014 Question
3. List the animal’s name, breed name, and weight for all animals. 
   Using the male and female high and low weight averages, 
   determine whether the animal’s weight falls within the low and high 
   average weights for their own gender and breed. Indicate whether the animal 
   is Underweight, Normal, or Overweight. Format the columns and headings as 
   shown below, and order by The animal’s weight descending.
*/
...
-- Query 4
/* 2016 Question
4. What is the most popular tube type requested in terms of the number of 
   tubes rented? (that is, it has the largest number of tubes rented.) 
   Display tube type ID, Name, Size, Person Capacity, Cooler Capacity, 
   Drink Capacity and the total number of tubes rented.
>> Summary:
	- limit: MAX(SUM(Trip_Equip.qty))
	- columns: tubetype, tubename, size, pplcapacity, coolercapacity, 
	  drink capacity, sum(trip_equip.qty)
*/

SELECT * 
FROM (SELECT DISTINCT a.Tube_Type_ID AS "Tube Type",
...
/* 2015 Question
4. Create a query that lists each customer’s ID number, 
...
/* 2014 Question
4. This query will help shelter personnel find adopters for animals that 
   they are currently sheltering. The shelter personnel will be prompted 
   to enter the breed of the animal. Any adopters that have indicated this 
   type of animal on their wishlist will be displayed. The breed of the 
   animal must match what the user has typed in, and the breed and gender 
   of the animal must match the breed and gender on the adopter’s wish list. 
   Show the adopter’s full name, desired breed, desired gender, name of the 
   animal, and gender of the animal. The query should work correctly regardless 
   of case, that is, BEAGLE, beagle, and Beagle should all work. Display the 
   gender with capital letters, i.e., “M” or “F”. Order by the first column.
*/
...
-- Query 5
/* 2015 Question
5. Select all products whose unit price is greater than the average 
   unit price for their own product line. Order by the product description.
*/
...
-- Alternative: unnecessary to do join on/group by in inner query 
SELECT prod_description AS "Product Description",
       prod_line_name AS "Product Line",
       TO_CHAR(outp.unit_price, '$9,999') AS "Unit Price",
       (SELECT TO_CHAR(AVG(inp.unit_price), '$9,999')
...
/* 2014 Question
5. Select all animals whose weight is an odd number that is more than the 
   heaviest cat. Include the animal’s name, weight, and the name of the 
   volunteer who is taking care of it. Indicate if there is no volunteer. 
   Order by weight from largest to smallest
*/
...
-- Tie-Breaker
/* 2016 Question
Q. What was the average travel time in minutes between destinations 3 and 4?
>> Summary:
...
/* 2015 Question
Q. This query will return the street address of any customer, such that 
   the numerical part of the street address contains the number of 
   characters in that street address.
*/
